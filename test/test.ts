...
describe("looksSame", () => {
    ...
    forFilesAndBuffers(getImage => {
        ...
        it("should return false if difference is not seen by human eye and strict mode is enabled", done => {
            looksSame(
                getImage("ref.png"),
                getImage("different-unnoticable.png"),
                { strict: true },
                (error, { equal }) => {
                    expect(error).to.equal(null);
                    expect(equal).to.equal(false);
        ...
        ["red", "blue", "green"].forEach(channel => {
        ...
        forFilesAndBuffers(getImage => {
            describe("if passed areas have different sizes", () => {
                it('should return "false"', done => {
                    looksSame(
                        {
                            source: getImage("bounding-box-diff-1.png"),
                            boundingBox: { left: 1, top: 1, right: 2, bottom: 1 },
                        },
                        {
                            source: getImage("bounding-box-diff-1.png"),
                            boundingBox: { left: 5, top: 5, right: 5, bottom: 6 },
                        },
                        (error, { equal }) => {
                            assert.isNull(error);
            ...
            describe("if passed areas have the same sizes but located in various places", () => {
                it("should return true if images are equal", done => {
                    looksSame(
                        {
                            source: getImage("bounding-box-diff-1.png"),
                            boundingBox: { left: 1, top: 1, right: 4, bottom: 4 },
                        },
                        {
                            source: getImage("bounding-box-diff-2.png"),
                            boundingBox: { left: 5, top: 5, right: 8, bottom: 8 },
                        },
                        (error, { equal }) => {
                            assert.isNull(error);
                            assert.isTrue(equal);
                ...
                it("should return false if images are different", done => {
                    looksSame(
                        {
                            source: getImage("bounding-box-ref-1.png"),
                            boundingBox: { left: 1, top: 1, right: 4, bottom: 4 },
                        },
                        {
                            source: getImage("bounding-box-ref-2.png"),
                            boundingBox: { left: 5, top: 5, right: 8, bottom: 8 },
                        },
                        (error, { equal }) => {
                            assert.isNull(error);
    ...
    describe("with ignoreCaret", () => {
        forFilesAndBuffers(getImage => {
            ...
            it("if enabled, should return true for images with caret intersecting with a letter", done => {
                looksSame(
                    getImage("no-caret+text.png"),
                    getImage("caret+text.png"),
                    { ignoreCaret: true },
                    (error, { equal }) => {
                        expect(error).to.equal(null);
                        expect(equal).to.equal(true);
            ...
            it("if enabled, should return true for images with caret and antialiased pixels", done => {
                ...
                looksSame(
                    getImage("caret+antialiasing.png"),
                    getImage("no-caret+antialiasing.png"),
                    opts,
                    (error, { equal }) => {
                        expect(error).to.equal(null);
                        expect(equal).to.equal(true);
...
describe("createDiff", () => {
    ...
    describe("with comparing by areas", () => {
        it("should create diff image equal to reference", done => {
            looksSame.createDiff(
                {
                    reference: {
                        source: srcPath("bounding-box-ref-1.png"),
                        boundingBox: { left: 1, top: 1, right: 4, bottom: 4 },
                    },
                    current: {
                        source: srcPath("bounding-box-ref-2.png"),
                        boundingBox: { left: 5, top: 5, right: 8, bottom: 8 },
                    },
                    diff: tempName,
                    highlightColor: "#FF00FF",
                ...
                () => {
                    looksSame(
                        {
                            source: srcPath("bounding-box-diff-1.png"),
                            boundingBox: { left: 1, top: 1, right: 4, bottom: 4 },
                        },
                        tempName,
                        (error, { equal }) => {
    ...
    describe("with antialiasing", () => {
        ...
        it("should create diff image not equal to reference if there is diff not in antialised pixels", done => {
            looksSame.createDiff(
                ...
                () => {
                    looksSame(srcPath("antialiasing-ref.png"), tempName, (error, { equal }) => {
                        expect(error).to.equal(null);
                        expect(equal).to.equal(false);
                        done();
                    });
    ...
    describe("with ignoreCaret", () => {
        describe("if there is only diff in caret", () => {
            it("should create diff image equal to reference if ignore caret is not set", done => {
                looksSame.createDiff(
                    ...
                    () => {
                        looksSame(srcPath("no-caret.png"), tempName, { ignoreCaret: false }, (error, { equal }) => {
                            expect(error).to.equal(null);
                            expect(equal).to.equal(true);
                            done();
                        });
        ...
        it("should create diff image not equal to reference if there is diff not in caret", done => {
            looksSame.createDiff(
                ...
                () => {
                    looksSame(srcPath("no-caret.png"), tempName, (error, { equal }) => {
                        expect(error).to.equal(null);
                        expect(equal).to.equal(false);
                        done();
                    });
    ...
    it("should create diff image equal to reference if there are diff in antialised pixels and caret", done => {
        looksSame.createDiff(
            ...
            () => {
                looksSame(
                    srcPath("caret+antialiasing.png"),
                    tempName,
                    { ignoreAntialiasing: false },
                    (error, { equal }) => {
                        expect(error).to.equal(null);
...
describe("colors", () => {
    it("should return true for same colors", () => {
        expect(looksSame.colors({ R: 255, G: 0, B: 0 }, { R: 255, G: 0, B: 0 })).to.be.equal(true);
    });

    it("should return false for different colors", () => {
        expect(looksSame.colors({ R: 255, G: 0, B: 0 }, { R: 0, G: 0, B: 255 })).to.be.equal(false);
    });

    it("should return true for similar colors", () => {
        expect(looksSame.colors({ R: 255, G: 0, B: 0 }, { R: 254, G: 1, B: 1 })).to.be.equal(true);
    });

    it("should return false for similar colors if tolerance is low enough", () => {
        expect(looksSame.colors({ R: 255, G: 0, B: 0 }, { R: 254, G: 1, B: 1 }, { tolerance: 0.0 })).to.be.equal(false);
    });

    it("should return true for different colors if tolerance is high enough", () => {
        expect(looksSame.colors({ R: 255, G: 0, B: 0 }, { R: 0, G: 0, B: 255 }, { tolerance: 55.0 })).to.be.equal(true);
    });
});
...
describe("getDiffArea", () => {
    ...
    it("should return correct diff area for different images", done => {
        looksSame.getDiffArea(
            srcPath("ref.png"),
            srcPath("different.png"),
