...
exports.calcDiffImage = async (img1, img2, comparator, highlightColor, clustersSize) => {
    const diffColor = exports.parseColorString(highlightColor || '#ff0000');
    ...
    const smallerImage = img1.height * img1.width < img2.height * img2.width ? img1 : img2;

    const diffBuffer = Buffer.alloc(maxHeight * maxWidth * diffChannels);
    const diffArea = new DiffArea();
    const diffClusters = new DiffClusters(clustersSize);

    let differentPixels = totalPixels - smallerImage.height * smallerImage.width;
    let diffBufferPos = 0;
    ...
    const diffImage = await img.fromBuffer(diffBuffer, {raw: {width: maxWidth, height: maxHeight, channels: diffChannels}});
