...
## Getting diff bounds
Looksame returns information about diff bounds. It returns only first pixel
if you passed `findAllDiff` option with `false` value. The whole diff area
would be returned if `findAllDiff` option is not passed or it's passed with
`true` value.

```javascript
looksSame('image1.png', 'image2.png', {findAllDiff: true}, function(error, {equal, diffBounds}) {
    // {
    //     equal: false,
